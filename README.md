...
> **使用者請看 [docs/使用說明.md](docs/使用說明.md)** —— 安裝、操作、常見問題。
> 這份 README 是給開發者的。
...
- **📁 零摩擦掛載系統 (File System Access)**
  - 徹底捨棄繁瑣的單檔匯入。只要點擊 `選擇遊戲資料夾` 選取大富翁2的遊戲主目錄，編輯器便會自動深入子資料夾尋找並解析 `MAP/MAP.PAK` 與 `SAVE/RICH2.DSK`。
  - 編輯完成後，點擊 `儲存到遊戲` 即可無縫複寫回原資料夾，存檔即生效。
